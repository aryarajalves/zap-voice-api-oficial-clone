...
async function run() {
  ...
  try {
    ...
    // Tirar print do modal aberto mostrando a nova barra com filtro de Status
    console.log('5. Capturando tela com a prova visual da barra de filtros...');
    const screenshotPath = path.join(dir, 'disparos_filtro_status.png');
    ...
    await page.screenshot({ path: path.join(dir, 'filters_error.png') }).catch(() => {});
  } finally {
    browser.close();
  }
}
