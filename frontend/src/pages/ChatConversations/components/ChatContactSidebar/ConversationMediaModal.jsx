...
export default function ConversationMediaModal({
    ...
    const linksList = mediaData?.links || [];
    ...
    // Obter lista atual com base na aba ativa
        ...
        <div className="fixed inset-0 z-[999] flex items-center justify-center p-3 sm:p-6 md:p-8">
            ...
            <div className="relative w-full max-w-4xl bg-[#0f172a] border border-white/10 rounded-2xl shadow-2xl overflow-hidden flex flex-col h-[85vh] z-10 animate-in fade-in zoom-in-95 duration-200">
                                    ...
                                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                                        {paginatedItems.map((item) => {
                                            ...
                                            return (
                                                <div
                                                    key={item.id}
                                                    className="group relative aspect-square bg-[#1e293b]/70 border border-white/10 rounded-xl overflow-hidden cursor-pointer hover:border-emerald-500/50 hover:shadow-lg transition-all"
                                                ...
                                                >
                                                    {item.type === 'video' ? (
                                                        ...
                                                        <>
                                                            ...
                                                            />
                                                            <div className="w-full h-full hidden flex-col items-center justify-center bg-[#1e293b] text-gray-400 p-2 text-center">
                                                    ...
                                                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-2 text-white">
                                                        <p className="text-[10px] font-medium truncate">{item.caption || (item.type === 'video' ? 'Vídeo' : 'Imagem')}</p>
                                                        <p className="text-[9px] text-gray-300">{formatDate(item.timestamp)}</p>
                                                    </div>
                                                </div>
                                            );
                                        })}
                                ...
                                paginatedItems.length > 0 ? (
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                        {paginatedItems.map((doc) => {
                                            const resolvedUrl = resolveMediaUrl(doc.url, activeClientId);
                                            return (
                                                <a
                                                    key={doc.id}
                                                    href={resolvedUrl}
                                                ...
                                                >
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        <p className="text-xs font-semibold text-gray-200 truncate group-hover:text-emerald-400 transition-colors">
                                                            {doc.filename || (doc.url ? doc.url.split('/').pop().split('?')[0] : 'Documento')}
                                                        </p>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 px-6 py-3.5 border-t border-white/10 bg-[#1e293b]/80">
                    {/* Indicador e Controles de Paginação (Máx 20 itens por página) */}
                    <div className="flex items-center gap-3">
                        {totalItems > 0 && (
