      ...
      <div className="bg-indigo-500/5 dark:bg-indigo-500/[0.03] border border-indigo-500/10 rounded-2xl p-5 space-y-4">
        ...
        {mapping.manychat_active && (
          <div className="space-y-4 pt-2 animate-in slide-in-from-top-2 duration-300">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-gray-100/10 pt-4">
              <div className="space-y-1.5">
                <label className="text-[9px] font-black text-gray-400 uppercase tracking-widest px-1">Data de Corte (Etiqueta Alternativa)</label>
                <input
                  type="datetime-local"
                  value={formatDateTimeLocal(mapping.manychat_start_date)}
                  onChange={(e) => {
                    const localVal = e.target.value;
                    if (!localVal) {
                      updateMapping(mIndex, 'manychat_start_date', null);
                    } else {
                      updateMapping(mIndex, 'manychat_start_date', new Date(localVal).toISOString());
                    }
                  }}
              ...
              <div className="space-y-1.5">
                <label className="text-[9px] font-black text-gray-400 uppercase tracking-widest px-1">Etiqueta Alternativa (A partir da data)</label>
                <div className="relative group">
                  <div className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-[10px]">+</div>
                  <input
                    type="text"
                    value={mapping.manychat_tag_alternative || ''}
                    onChange={(e) => updateMapping(mIndex, 'manychat_tag_alternative', e.target.value)}
                    className="w-full bg-white dark:bg-[#0b1120] border border-gray-100 dark:border-white/5 rounded-lg pl-8 pr-3 py-2 text-[11px] font-bold text-gray-700 dark:text-gray-200 outline-none focus:ring-1 focus:ring-indigo-500/30 shadow-inner"
                    placeholder="Tag Alternativa..."
                  />
                </div>
              </div>
            </div>
