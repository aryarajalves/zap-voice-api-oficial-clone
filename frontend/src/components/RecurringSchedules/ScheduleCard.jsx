    ...
    return (
        ...
        >
            ...
            <div className="relative z-10 flex flex-col h-full space-y-6">
                ...
                <div className="grid grid-cols-6 gap-1.5 pt-4 border-t border-white/5">
                    <button 
                        onClick={() => onViewMessage(schedule)}
                        className="w-full h-10 bg-purple-500/10 hover:bg-purple-500 text-purple-400 hover:text-white rounded-xl flex items-center justify-center transition-all active:scale-90 border border-purple-500/20 shadow-lg"
                    ...
                    <button 
                        ...
                        className="w-full h-10 bg-amber-500/10 hover:bg-amber-500 text-amber-500 hover:text-white rounded-xl flex items-center justify-center transition-all active:scale-90 border border-amber-500/20 shadow-lg"
                        title="Disparar Agora (Manual 1 vez)"
                        disabled={isTriggering}
                    >
                        <FiZap size={15} className={isTriggering ? 'animate-pulse' : ''} />
                    </button>
                    <button 
                        onClick={() => onFetchContacts(schedule.id)}
                        className="w-full h-10 bg-blue-500/10 hover:bg-blue-500 text-blue-400 hover:text-white rounded-xl flex items-center justify-center transition-all active:scale-90 border border-blue-500/20 shadow-lg"
                        title="Ver Contatos"
                    >
                        <FiUsers size={15} />
                    </button>
                    <button 
                        ...
                        className="w-full h-10 bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-white rounded-xl flex items-center justify-center transition-all active:scale-90 border border-white/5 shadow-lg"
                    ...
                    <button 
                        ...
                        className={`w-full h-10 rounded-xl flex items-center justify-center transition-all active:scale-90 border shadow-lg ${schedule.is_active ? 'bg-white text-slate-900 border-white hover:bg-slate-100' : 'bg-emerald-500 text-white border-emerald-400 hover:bg-emerald-400'}`}
                        title={schedule.is_active ? 'Desativar' : 'Ativar'}
                    >
                        {schedule.is_active ? <FiXCircle size={16} /> : <FiPlay size={16} />}
                    </button>
                    <button 
                        onClick={() => onConfirmDelete(schedule)}
                        className="w-full h-10 bg-slate-800 hover:bg-red-500 text-slate-400 hover:text-white rounded-xl flex items-center justify-center transition-all active:scale-90 border border-white/5 shadow-lg"
                        title="Excluir"
                    >
                        <FiTrash2 size={15} />
                    </button>
