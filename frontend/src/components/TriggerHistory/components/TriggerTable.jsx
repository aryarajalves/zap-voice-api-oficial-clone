    ...
    return (
        <tr className={`hover:bg-gray-50 dark:hover:bg-gray-700/30 transition ${selectedIds.includes(trigger?.id) ? 'bg-blue-50 dark:bg-blue-900/10' : ''}`}>
            ...
            <td className="p-4 text-right flex justify-end gap-2">
                {(trigger.status === 'pending' || (trigger.status === 'processing' && !trigger.is_bulk)) ? (
                    <>
                    ...
                    <div className="flex items-center gap-2">
                        {trigger.status === 'failed' && (
                            <button onClick={() => handleRetry(trigger.id)} className="p-1 text-blue-600 hover:bg-blue-50 rounded" title="Reenviar"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg></button>
                        )}
                        {user?.role === 'super_admin' && (
                            <button onClick={() => handleDelete(trigger.id)} className="p-1 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded" title="Excluir"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                        )}
                    </div>
                )}
            </td>
