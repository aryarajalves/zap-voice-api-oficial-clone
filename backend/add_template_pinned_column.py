...
def migrate_template_pinned_column():
    db = SessionLocal()
    try:
        # Tenta adicionar is_pinned
        try:
            db.execute(text("ALTER TABLE whatsapp_template_cache ADD COLUMN is_pinned BOOLEAN NOT NULL DEFAULT FALSE"))
            db.commit()
            print("✅ Coluna is_pinned adicionada com sucesso na tabela whatsapp_template_cache.")
        except Exception as e:
            db.rollback()
            if "already exists" in str(e) or "duplicate column" in str(e):
                print("ℹ️ Coluna is_pinned já existe na tabela whatsapp_template_cache.")
            else:
...
if __name__ == "__main__":
    migrate_template_pinned_column()
