...
def migrate_funnel_columns():
    db = SessionLocal()
    try:
        # Tenta adicionar is_archived
        try:
            db.execute(text("ALTER TABLE funnels ADD COLUMN is_archived BOOLEAN NOT NULL DEFAULT FALSE"))
            db.commit()
            print("✅ Coluna is_archived adicionada com sucesso.")
        except Exception as e:
            db.rollback()
            if "already exists" in str(e) or "duplicate column" in str(e):
                print("ℹ️ Coluna is_archived já existe no banco de dados.")
            else:
                print(f"❌ Erro ao adicionar coluna is_archived: {e}")

        # Tenta adicionar tag
        try:
            db.execute(text("ALTER TABLE funnels ADD COLUMN tag VARCHAR"))
            db.commit()
            print("✅ Coluna tag adicionada com sucesso.")
        except Exception as e:
            db.rollback()
            if "already exists" in str(e) or "duplicate column" in str(e):
                print("ℹ️ Coluna tag já existe no banco de dados.")
            else:
                print(f"❌ Erro ao adicionar coluna tag: {e}")

    finally:
        db.close()

if __name__ == "__main__":
    migrate_funnel_columns()
